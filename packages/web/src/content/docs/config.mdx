...
You can pin stable user preferences directly into the model context with the
`preferences` option. Use this for personal style, workflow, and tool choices
that should apply across sessions without creating a separate instruction file.
...
`preferences` can be a single string or an array of strings. When multiple
config files define preferences, OpenCode merges and deduplicates them.
