...
When any connected server is deferred, OpenCode exposes two small tools instead:

- `mcp_search` searches deferred MCP server names, tool names, and descriptions.
- `mcp_load` activates selected tools. Loaded tools are available on the next assistant step.
