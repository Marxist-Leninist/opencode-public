...
Large MCP servers can expose dozens or hundreds of tools. Set `defer` to `true` when you want OpenCode to keep that server's full tool catalog out of the model context until the agent needs it.

```jsonc title="opencode.jsonc" {7}
{
...
When any connected server is deferred, OpenCode exposes one small discovery tool instead:

- `mcp_search` searches deferred MCP server names, tool names, and descriptions.
- Matching tools are automatically activated and become available on the next assistant step.
- Pass `dry_run: true` to inspect matches without activating them.

`mcp_search` also accepts compact query operators:

- `select:server_tool1,server_tool2` loads exact tool ids without ranking.
- `+token` requires that token to appear in the server name, tool name, or description.

`mcp_search` supports three modes:

| Mode       | Description                                                                                      |
| ---------- | ------------------------------------------------------------------------------------------------ |
| `smart`    | Default. Uses local query expansion and ranking without spending model tokens.                   |
| `standard` | Strict token matching for deterministic searches.                                                |
| `augment`  | Uses the smart results as candidates, then reranks them with a configured or per-call LLM model. |

You can also make every MCP server deferred by default:
...
Set `"defer": false` on a specific server to opt it out of the global default. The configured `model` is only used when the search mode is `augment`; `smart` remains the default so discovery does not spend model tokens unless requested. The configured `limit` controls how many matching tool schemas are activated by each search.
...
| `defer`       | Boolean |          | Search/load this server's tools on demand instead of registering every tool up front. |
