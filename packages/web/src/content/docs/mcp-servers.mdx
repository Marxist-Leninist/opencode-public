...
Set `"defer": false` on a specific server to opt it out of the global default. The configured `model` is only used when the search mode is `augment`; `smart` remains the default so discovery does not spend model tokens unless requested.
