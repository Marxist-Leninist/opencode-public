...
function platformLauncher(target: string, reveal: boolean, kind: TargetKind, platform: NodeJS.Platform) {
  if (platform === "win32") {
    ...
    // `start ""` makes the empty-string "title" so the next quoted arg is treated as the path.
    return { cmd: process.env.ComSpec ?? "cmd.exe", args: ["/c", "start", "", target] }
  }
  if (platform === "darwin") {
    ...
    return {
      ...
      execute: (params: Params, ctx: Tool.Context<Metadata>) =>
        Effect.gen(function* () {
          ...
          // explorer.exe /select,<path> exits with code 1 even on success.
          // Treat code 0 OR (explorer + windows + reveal) as delivered.
          const delivered =
            result.code === 0 ||
            (platform === "win32" && reveal && kind === "file" && launcher.cmd === "explorer.exe" && result.code !== null)
